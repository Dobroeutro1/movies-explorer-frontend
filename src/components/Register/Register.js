...
class Register extends React.PureComponent {
  ...
  handleReady = () => {
    return this.setState((prev) => {
      const ready = ValidationForm('email', this.state.email.value) && ValidationForm('password', this.state.password.value) && ValidationForm('name', this.state.name.value)
  ...
  handleSubmit = (e) => {
    ...
    this.props.onRegister(this.state.email.value, this.state.password.value, this.state.name.value)
  }
      ...
      <div className='register'>
        ...
        <form className='register__form'>
          <div className='register__form_element'>
            ...
            <input onChange={this.handleChange} required type='text' className='register__input' id='name'></input>
            <span className={`register__span ${this.state.name.valid ? '' : 'opened'}`}>Что-то пошло не так...</span>
          </div>
          ...
          <span className={`error ${this.props.errorMessage.type === 'register' ? '' : 'opened'}`}>{this.props.errorMessage.value}</span>
          <button className={`register__btn ${this.state.ready ? '' : 'disabled'}`} type='submit' onClick={this.handleSubmit} disabled={!this.state.ready}>
            Зарегистрироваться
          </button>
        </form>
