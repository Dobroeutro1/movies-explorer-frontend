...
class MoviesCard extends React.PureComponent {
  ...
  addMovie = () => {
    ...
    this.props.addMovie(country, director, duration, year, description, image, trailer, thumbnail, nameRU, nameEN, id)
    ...
    return (
      <div className='card'>
        <div className='card__header'>
          <a target='_blank' href={this.props.movie.trailerLink ? this.props.movie.trailerLink : 'youtube.com'} className='card__title' rel='noreferrer'>
            {this.props.movie.nameRU}
          </a>
          <p className='card__duration'>{this.props.movie.duration} минут</p>
        </div>
        <img
          src={`${this.props.movie.image ? (this.props.movie.image.url ? `https://api.nomoreparties.co${this.props.movie.image?.url}` : this.props.movie.image) : ''}`}
        ...
        {this.state.added && this.props.path === '/movies' ? <button onClick={this.deleteMovie} className='card__btn card__btn_checked'></button> : null}
